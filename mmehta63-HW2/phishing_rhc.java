    ...
    public static void main(String[] args) {
        ...
        for (int trainingIterations : iterations) {
            ...
            for (int i = 0; i < oa.length; i++) {
                ...
                double train_error = train(oa[i], networks[i], oaNames[i], trainingIterations); //trainer.train();
                end = System.nanoTime();
                ...
                results += "\nTrain Results for " + oaNames[i] + ": \nCorrectly classified " + correct + " instances." +
                        ...
                        + df.format(correct * 100.0 / ((correct + incorrect))) + "%\nTraining time: " + df.format(trainingTime)
                        + " seconds\nValidation F1 Score: " + df.format(2.0*tp/(2*tp + fn + fp)) + "\n";

                final_result = oaNames[i] + ",train," + trainingIterations + "," + "training accuracy" + "," + df.format(correct * 100.0 / ((correct + incorrect)))
                        + "," + "training error" + "," + df.format(train_error)
                ...
                final_result = oaNames[i] + ",test," + trainingIterations + "," + "testing accuracy" + "," + df.format(correct * 100.0 / ((correct + incorrect)))
                        + "," + "training error" + "," + df.format(train_error)
                        + "," + "training time" + "," + df.format(trainingTime) + "," + "testing time" +
                        "," + df.format(testingTime) + "," + "f1 score" + "," + df.format(2.0*tp/(2*tp + fn + fp));
                write_output_to_file("Optimization_Results", "phishing_results_rhc.csv", final_result, true);
